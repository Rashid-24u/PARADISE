...
function Fees() {
  ...
  const getTotalBalance = () => {
    return fees.reduce((sum, fee) => {
      ...
      return sum + (fee.total_amount - paid);
    }, 0);
  };
    ...
    try {
      ...
      if (res.ok) {
        ...
        setReceiptData({
          ...
          fee: fee,
          total_paid: getPaid(fee.id) + Number(paymentAmount),
          remaining: fee.total_amount - (getPaid(fee.id) + Number(paymentAmount)),
        });
        ...
        Promise.all([
          ...
          setFees(f);
          setPayments(p);
          const updatedFee = f.find((x) => x.id === fee.id);
          setSelectedFee(updatedFee || fee);
        });
        setTimeout(() => setShowSuccess(""), 3000);
      } else {
        setShowError("Error: " + JSON.stringify(data));
        setTimeout(() => setShowError(""), 2500);
      }
    } catch (err) {
  ...
  const printPaymentReceipt = (payment, student, fee) => {
    ...
    const remaining = fee.total_amount - totalPaid;
    ...
    printWindow.document.write(`
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .school-name { font-size: 28px; font-weight: bold; letter-spacing: 1px; }
          .school-address { font-size: 12px; margin-top: 5px; opacity: 0.9; }
          .receipt-title { font-size: 24px; margin-top: 15px; font-weight: 600; }
          .receipt-body { padding: 30px; }
          .student-info { background: #f8f9fa; padding: 20px; border-radius: 12px; margin-bottom: 25px; }
          .student-info h3 { color: #1a472a; margin-bottom: 15px; border-bottom: 2px solid #d4af37; display: inline-block; }
          ...
          .payment-table th { background: #f0f0f0; padding: 12px; text-align: left; border-bottom: 2px solid #ddd; }
          .payment-table td { padding: 10px 12px; border-bottom: 1px solid #eee; }
          ...
          .receipt-footer { background: #f8f9fa; padding: 20px; text-align: center; border-top: 1px solid #eee; font-size: 12px; color: #666; }
      ...
      <body>
        <div class="receipt">
          ...
          <div class="receipt-body">
            <div class="student-info">
              ...
              <div class="info-grid">
                <div class="info-label">Student Name:</div><div class="info-value">${student?.name || ''}</div>
                <div class="info-label">Admission No:</div><div class="info-value">${student?.admission_no || ''}</div>
                <div class="info-label">Course/Class:</div><div class="info-value">${getCourseName(student?.course) || ''}</div>
                <div class="info-label">Parent Name:</div><div class="info-value">${student?.parent_name || 'N/A'}</div>
              </div>
            </div>
            <div class="payment-details">
              ...
              <div class="amount-row">
                <div class="amount-item"><strong>Total Fee:</strong> <span>₹${fee.total_amount.toLocaleString()}</span></div>
                <div class="amount-item"><strong>Previous Paid:</strong> <span>₹${(totalPaid - payment.amount).toLocaleString()}</span></div>
                <div class="amount-item"><strong>Current Payment:</strong> <span>₹${payment.amount.toLocaleString()}</span></div>
                <div class="amount-item amount-total"><strong>Total Paid:</strong> <span>₹${totalPaid.toLocaleString()}</span></div>
                <div class="amount-item"><strong>Remaining Balance:</strong> <span>₹${remaining.toLocaleString()}</span></div>
              </div>
            </div>
            <div class="signature">
        ...
        <div style="text-align: center; margin-top: 20px;">
          <button onclick="window.print();" style="padding: 10px 20px; background: #1a472a; color: white; border: none; border-radius: 8px; cursor: pointer;">🖨️ Print Receipt</button>
        </div>
      </body>
      </html>
    `);
            ...
            <div style={styles.receiptPreview}>
              <div style={styles.receiptCard}>
                ...
                <div style={styles.receiptContent}>
                  ...
                  <p><strong>Amount Paid:</strong> <span style={{color: "#22c55e", fontSize: "20px", fontWeight: "bold"}}>₹{receiptData.amount.toLocaleString()}</span></p>
                  <p><strong>Remarks:</strong> {receiptData.remarks || "Fee Payment"}</p>
                  <div style={styles.receiptBalance}>
                    <span>Total Paid: ₹{receiptData.total_paid.toLocaleString()}</span>
                    <span>Balance: ₹{receiptData.remaining.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div style={styles.mainGrid}>
        ...
        <div style={styles.leftPanel}>
          ...
          <div style={styles.summaryRow}>
            ...
            <div style={styles.summaryCard}><div style={styles.summaryIcon}>✅</div><div style={styles.summaryContent}><div style={styles.summaryLabel}>Fully Paid</div><div style={styles.summaryValue}>{filteredFees.filter((f) => f.status === "Paid").length}</div></div></div>
            ...
            <div style={styles.summaryCard}><div style={styles.summaryIcon}>⚠️</div><div style={styles.summaryContent}><div style={styles.summaryLabel}>Total Balance Pending</div><div style={styles.summaryValue} styles={{color: "#f97316"}}>₹{totalBalance.toLocaleString()}</div></div></div>
          </div>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {filteredFees.map((f) => {
                    ...
                    const balance = f.total_amount - paid;
                    ...
                    return (
                      <tr key={f.id} style={selectedFee && selectedFee.id === f.id ? styles.rowActive : styles.tableRow} onClick={() => { setSelectedStudentId(String(f.student)); setSelectedStudent(student); setSelectedFee(f); setFeeTotalEdit(f.total_amount); setErrors({}); }}>
                        ...
                        <td style={styles.td}><span style={f.status === "Paid" ? styles.statusPaid : styles.statusPending}>{f.status === "Paid" ? "✅ Paid" : "⏳ Pending"}</span>{f.status !== "Paid" && (<div style={styles.progressBar}><div style={{...styles.progressFill, width: `${paidPercent}%`}} /></div>)}</td>
                      </tr>
                    );
                  })}
        ...
        {/* RIGHT SIDE - Student Fee Details */}
            ...
            <>
              ...
              <div style={styles.feeSummaryCard}>
                <div style={styles.feeSummaryRow}>
                  <div style={styles.feeSummaryItem}><div style={styles.feeSummaryLabel}>Total Fee</div><div style={styles.feeSummaryValue}>₹{selectedFee.total_amount.toLocaleString()}</div></div>
                  ...
                  <div style={styles.feeSummaryItem}><div style={styles.feeSummaryLabel}>Remaining Balance</div><div style={getPaid(selectedFee.id) === selectedFee.total_amount ? styles.feeSummaryValuePaid : styles.feeSummaryValuePending}>₹{(selectedFee.total_amount - getPaid(selectedFee.id)).toLocaleString()}</div></div>
                </div>
                <div style={styles.statusBadgeLarge(selectedFee.status === "Paid")}>{selectedFee.status === "Paid" ? "🎉 Fully Paid" : `⚠️ Pending: ₹${(selectedFee.total_amount - getPaid(selectedFee.id)).toLocaleString()} remaining`}</div>
              </div>

              <div style={styles.formCard}>
                <label style={styles.label}>Update Total Fee Amount</label>
                <div style={styles.inputGroup}><span style={styles.inputGroupPrefix}>₹</span><input style={{...styles.input, ...styles.inputGroupField, borderColor: errors.total ? "#dc3545" : "#e2e8f0"}} type="number" placeholder="Enter total amount" value={feeTotalEdit} onChange={(e) => setFeeTotalEdit(e.target.value)} /><button style={styles.secondaryBtn} onClick={updateFeeTotal}>Update</button></div>
                {errors.total && <span style={styles.errorText}>{errors.total}</span>}
              </div>

              <div style={styles.formCard}>
                <label style={styles.label}>Add New Payment</label>
                <div style={styles.inputGroup}><span style={styles.inputGroupPrefix}>₹</span><input style={{...styles.input, ...styles.inputGroupField, borderColor: errors.amount ? "#dc3545" : "#e2e8f0"}} type="number" placeholder="Enter payment amount" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} /><button style={styles.primaryBtn} onClick={addPayment}>+ Add Payment</button></div>
                ...
                <div style={styles.historyList}>
                  {payments.filter((p) => p.fees === selectedFee.id).sort((a, b) => new Date(b.date) - new Date(a.date)).map((p) => (
                    <div key={p.id} style={styles.historyItem}>
                      ...
                      <div style={styles.historyAmount}>+ ₹{p.amount.toLocaleString()}</div>
                      <button 
                        style={styles.receiptHistoryBtn}
                        onClick={() => printPaymentReceipt(p, selectedStudent, selectedFee)}
                      >
                        🧾 Receipt
                      </button>
                    </div>
...
const styles = {
  ...
  headerStats: { background: "white", padding: "12px 20px", borderRadius: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" },
  headerStat: { display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "4px", span: { fontSize: "12px", color: "#64748b" }, strong: { fontSize: "20px", fontWeight: "700", color: "#22c55e" } },
  ...
  summaryRow: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px" },
  ...
  summaryIcon: { fontSize: "32px" }, summaryContent: { flex: 1 }, summaryLabel: { fontSize: "12px", color: "#64748b", fontWeight: "500" }, summaryValue: { fontSize: "24px", fontWeight: "700", color: "#1e293b" },
  ...
  tableRow: { cursor: "pointer", transition: "background 0.2s", borderBottom: "1px solid #f1f5f9" },
  ...
  studentName: { fontWeight: "600", color: "#1e293b" }, studentId: { fontSize: "11px", color: "#94a3b8" },
  ...
  statusBadgeLarge: (paid) => ({ textAlign: "center", padding: "10px", borderRadius: "12px", background: paid ? "#dcfce7" : "#ffedd5", color: paid ? "#166534" : "#9a3412", fontSize: "13px", fontWeight: "600" }),
  ...
  inputGroupField: { flex: 1 }, primaryBtn: { padding: "10px 20px", borderRadius: "12px", border: "none", background: "linear-gradient(135deg, #22c55e 0%, #16a34a 100%)", color: "#fff", fontSize: "13px", fontWeight: "600", cursor: "pointer", whiteSpace: "nowrap" },
  secondaryBtn: { padding: "10px 20px", borderRadius: "12px", border: "1px solid #e2e8f0", background: "white", color: "#475569", fontSize: "13px", fontWeight: "500", cursor: "pointer" },
  ...
  historyItem: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px", background: "#f8fafc", borderRadius: "12px", marginBottom: "8px", gap: "8px" },
  ...
  downloadBtn: { padding: "8px 16px", background: "#d4af37", color: "#1a472a", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "bold" },
